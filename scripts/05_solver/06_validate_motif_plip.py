...
def _find_plip_ligand_pdb(outdir: Path) -> Path:
    # PLIP output naming differs across versions:
    # - plipfixed.<stem>_<rand>.pdb
    # - <stem>_protonated.pdb
    for pattern in ("plipfixed.*.pdb", "*_protonated.pdb"):
        try:
            return _find_latest(outdir, pattern)
        except FileNotFoundError:
            continue
    raise FileNotFoundError(f"Expected PLIP ligand PDB in {outdir} (tried plipfixed.*.pdb and *_protonated.pdb)")
