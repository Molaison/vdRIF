...
- `outputs/05_solver/MTX_motif_debug_clash_validation.json`
- `outputs/05_solver/MTX_motif_debug_internal_clash_validation.json`

Non-debug run:

```bash
TIME_LIMIT_S=300 OBJECTIVE_MODE=balanced TARGET_RES=12 \
SITE_DIVERSITY_REWARD=2000 TARGET_RES_PENALTY=1200 \
MIN_UNIQUE_SITES=6 MAX_PER_SITE=2 CA_PREFILTER=12.0 \
...
Balanced solver parameter sweep on a fixed candidate set (writes ranked `report.json` + `report.md`):

```bash
bash scripts/99_harness/06_run_mtx_balanced_solver_sweep.sh
```

By default, sweep feasibility requires internal clash validation to pass (`scripts/05_solver/05_validate_motif_internal_clashes.py`). Disable only for debugging with `--no-enforce-internal-clash`.

Toy-data sweep example using the synthetic candidates generated by the previous smoke:

```bash
CANDIDATES_NPZ=processed/99_harness/solver_balanced_objective_<tag>/toy_candidates.npz \
CANDIDATES_META=processed/99_harness/solver_balanced_objective_<tag>/toy_candidates.json \
LIGAND_PDB=processed/99_harness/solver_balanced_objective_<tag>/LIG.pdb \
bash scripts/99_harness/06_run_mtx_balanced_solver_sweep.sh \
  --max-runs 12 --min-res 2 --max-res 6 --min-unique-sites-list 0,4
```
...
The solver wrappers above now run both clash validators automatically. Manual commands (for custom motif files) are:
